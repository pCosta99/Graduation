...
calcula :: Expr -> Int
calcula = cataExpr $ either id apply
  where apply (Op "+",(e1,e2)) = e1 + e2
        apply (Op "*",(e1,e2)) = e1 * e2

show' = cataExpr $ either show gather
  where gather = conc . ((conc . (closeLeft >< outOp)) >< closeRight) .  opToMiddle 
        outOp (Op s) = " " ++ s ++ " "
        closeRight l = l ++ ")"
        closeLeft l = '(' : l

compile :: String -> Codigo
compile = cataExpr conquer . (\x -> read x :: Expr)
...
divide s = if (length x /= length s && length y /= length s) then i2 (o, (t1, t2)) else i1 (read s :: Int) where
    ...
    (x,y) =  (f >< f) $ splitAt (improvedSpan s 0) s
    t1 = x
    t2 = tail y
    o = Op (singl $ head y)
...
soma_dimen H ((x1,y1),(x2,y2)) = (x1+x2, max y1 y2)
soma_dimen Hb ((x1,y1),(x2,y2)) = (x1+x2, max y1 y2)
soma_dimen Ht ((x1,y1),(x2,y2)) = (x1+x2, max y1 y2)
soma_dimen V ((x1,y1),(x2,y2)) = (max x1 x2, y1+y2)
soma_dimen Ve ((x1,y1),(x2,y2)) = (max x1 x2, y1+y2)
soma_dimen Vd ((x1,y1),(x2,y2)) = (max x1 x2, y1+y2)
...
alphaHt True (t,t') (x,y) = let ((x1,y1),(x2,y2)) = (dimen t, dimen t') in ((t,(x,y)),(t',(x+x1,y1-y2)))
alphaHt False (t,t') (x,y) = let ((x1,y1),(x2,y2)) = (dimen t, dimen t') in ((t,(x,y2-y1)),(t',(x+x1,y2)))
...
alphaH True (t,t') (x,y) = let ((x1,y1),(x2,y2)) = (dimen t, dimen t') in ((t,(x,y)),(t',(x1,(y1/2)-(y2/2))))
alphaH False (t,t') (x,y) = let ((x1,y1),(x2,y2)) = (dimen t, dimen t') in ((t,(x,(y2/2)-(y1/2))),(t',(x1,y)))

alphaV :: Bool -> (L2D,L2D) -> Origem -> ((L2D,Origem),(L2D,Origem))
alphaV True (t,t') (x,y) = let ((x1,y1),(x2,y2)) = (dimen t, dimen t') in ((t,(x,y)),(t',((x1/2)-(x2/2),y1)))
alphaV False (t,t') (x,y) = let ((x1,y1),(x2,y2)) = (dimen t, dimen t') in ((t,((x2/2)-(x1/2),y)),(t',(x,y1)))
...
nrPlusAllTrue = prj . aux where
   aux = cataList $ either init loop
   init _ = ([], True, True)
   loop v = (g v, h v, k v)
   prj (a,b,c) = (b,c)
   g(a,(t,b1,b2)) = (p1 a):t
   h(a,(t,b1,b2)) = not((p1 a) `elem` t) && b1
   k(a,(t,b1,b2)) = (p2 a) && b2 
...
auxJoin = anaList ((id -|- (aux >< id)) . (p1 -|- inPair) . distl . (outList >< id)) where
   aux = (split (p1 . p1) ((p1 -|- swap) . distl . (p2 >< id)))
   inPair = split (p1 >< id) (p2 >< id)

auxJoinCata :: ([(a, Either b c)],d) -> [(a, Either b (d,c))]
auxJoinCata = uncurry aux . swap
   where aux d = cataList (inList . (id -|- (g d >< id)))
