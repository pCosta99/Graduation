...
#define PATH_VENDAS_1M  "Vendas_1M"
#define PATH_VENDAS_3M  "Vendas_3M"
#define PATH_VENDAS_5M  "Vendas_5M"
...
Stats init_stats(char* statsFolder, int createDirectories, int active){
    ...
    if(!createDirectories){
        sprintf(totalPath, "mkdir -p %s/%s/%s", statsFolder, PATH_VENDAS_1M, PATH_PARSING);
        system(totalPath);
        sprintf(totalPath, "mkdir -p %s/%s/%s", statsFolder, PATH_VENDAS_1M, PATH_QUERIES);
        system(totalPath);
        sprintf(totalPath, "mkdir -p %s/%s/%s/%s", statsFolder, PATH_VENDAS_1M, PATH_QUERIES, "Query11");
        system(totalPath);
    }

/* --------------------------- PARSING ------------------------------- */
    key = my_strdup("loadCatalogClientes");
    sprintf(totalPath, "%s/%s/%s/%s", statsFolder, PATH_VENDAS_1M, PATH_PARSING, key);
...
/* --------------------------- QUERIES ------------------------------- */
    ...
    sprintf(totalPath, "%s/%s/%s/%s", statsFolder, PATH_VENDAS_1M, PATH_QUERIES, key);
    e = initEntry(fopen(totalPath, "a"));
    g_hash_table_insert(s->filesTable, key, e);
    ...
    sprintf(totalPath, "%s/%s/%s/%s/%s", statsFolder, PATH_VENDAS_1M, PATH_QUERIES, "Query11", key);
    e = initEntry(fopen(totalPath, "a"));
    g_hash_table_insert(s->filesTable, key, e);
