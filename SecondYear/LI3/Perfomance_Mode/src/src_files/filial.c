...
int getNCompras(CustomerBought cb, char type){
    int total = 0, i;
    for(i = 0; i < NUM_MESES; i++) total += cb->n_compras[type=='P'][i];
    return total;
}
...
float getAllProfit(CustomerBought cb, char type){
    float total = 0; int i;
    for(i = 0; i < NUM_MESES; i++) total += cb->total_gasto[type=='P'][i];
    return total;
}
...
void destroy_customerinfo(void* ptr){
    CustomerInfo ci = (CustomerInfo) ptr;
...
Filial init_filial(){
    ...
    f->branchCustomers = init_catalog(NUM_LETTERS, CUSTOMERS_DEPTH, 4000, 1000, destroy_customerinfo);
...
gboolean func(gpointer key, gpointer value, gpointer data){
    ...
    for(i = 0; i < NUM_MESES; i++) array[i] += (getNComprasMes(cb, 'P', i+1) + getNComprasMes(cb, 'N', i+1));
