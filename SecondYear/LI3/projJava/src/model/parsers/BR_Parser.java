...
public class BR_Parser implements IParser{
    private IConfigs configs;
    private int zero_value_sales;

    public BR_Parser(IConfigs configs){
        zero_value_sales = 0;
    ...
    public void readProducts(IGestVendasModel model, String path) throws IOException{
        try {
            ...
            while((line = br.readLine()) != null){
                if(validProduct(line)) {
                    model.addEmptyProductToFac(line);
                    model.addToProductCatalog(line);
                }
            }
        } catch(IOException e) {
    ...
    public void readCustomers(IGestVendasModel model, String path) throws IOException{
        try{
            ...
            // Get the lines, filter the valid ones and put them into the catalog
            while((line = br.readLine()) != null){
                if(validCustomer(line)) {
                    model.addToCustomerCatalog(line);
                    model.addEmptyCustomerToAllBranches(line);
                }
            }
        }catch(IOException e) {
            throw new IOException("No such file");
    ...
    public void readSales(IGestVendasModel model, String path) throws IOException{
        try{
            ...
            // Get the lines, filter the valid ones and put them into the catalog
            while((line = br.readLine()) != null){
                Sale sale = new Sale(line.split(" "));
                if(validSale(model, sale)){
                    model.billProduct(sale);
                    model.customerBoughtInBranch(sale);
                }
            }
            model.setZeroValueSales(zero_value_sales);
        }catch(IOException e) {
            throw new IOException("No such file");
        }
    }
