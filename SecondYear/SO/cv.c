...
int parser(char* buffer, char* path){
	...
	if(!strcmp(buffer, "reload") || command[0][0] == 'r') {
		...
		if(safeConversion(command[0], &artCode, NULL, 1)) {
			...
			/* Wait for permission to go read the output */
			pause();
			alarm(3);
			memset(out, '\0', 256);
			if((myfifo = open(path, O_RDONLY)) < 0){
				alarm(0);
		...
		if(safeConversion(command[0], &artCode, NULL, 1) || safeConversion(command[1], NULL, &price, 0)) {
			...
			char out[256];
			memset(out, '\0', 256);
			snprintf(out, 256, "%s %s %s\n", command[0], command[1], path);
			write(fifo, out, strlen(out)+1);
			/* Wait for permission to read the output */
			pause();
			alarm(3);
			memset(out, '\0', 256);
			if((myfifo = open(path, O_RDONLY)) < 0){
