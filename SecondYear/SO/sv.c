...
int process(int artCode, int quantity, char* path){
	struct artigo art;
	struct venda v;
	char out[256];
...
void checkStock(int artCode, char* path){
	char out[256];
	struct artigo art = readArtigo(artCode);
	int fds = open(STOCKS, O_RDONLY);
	int fdOut;
	long long inStock;
	float endPrice;
...
int parser(char* buffer){
	...
	if(!strcmp(command[0], "agregate")){
		...
		pidCliente = getPidFromPath(command[1]);
		if(artCode < code) checkStock(artCode, command[1]);
		else {
			int fdOut;
			if ((fdOut = open(command[1], O_WRONLY)) == -1){
				write(2, "couldn't open client fifo\n", 26);
			}
		...
		pidCliente = getPidFromPath(command[2]);
		if(artCode < code) process(artCode, quantity, command[2]);
		else {
			int fdOut;
			if ((fdOut = open(command[2], O_WRONLY)) == -1){
				write(2, "couldn't open client fifo\n", 26);
			}
			write(fdOut, "Invalid code!\n", 14);
			close(fdOut);
